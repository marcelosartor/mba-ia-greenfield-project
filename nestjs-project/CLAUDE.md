...
- `src/queue/` — `QueueModule`: the BullMQ queues `video-processing`, `video-processing-dlq` and `video-maintenance` (names in `queue.constants.ts`) and `VideoProcessingPublisher` (`publish` and `republish`; the job id is the video id)
...
- The size the client declares in `POST /videos` is stored (`declared_size_bytes`) and enforced, not just validated: part numbers beyond `ceil(size / part size)` are refused (`INVALID_PART_NUMBER`, 400), each presigned part URL has its exact `Content-Length` signed (the storage answers `403 SignatureDoesNotMatch` to any other length), and the completion accepts only as many parts and bytes as declared. Nothing else stops a client from sending more than it declared. Drafts created before the column existed have a null size and keep the older, unbounded behaviour until the sweeper removes them.
